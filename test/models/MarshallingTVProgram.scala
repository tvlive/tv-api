  ...
  "Write and reads" should {
    "transform TVProgram object to json" in {
      Json.toJson(TVProgram("bbc1", now, now.plusHours(2), Some(List("documentary")), Some(List("flags1")), Some(Serie("serie1", "ep1", None, None, None, None)), Some(Program("program1", None)), Some(id))).toString() mustBe
        s"""{"channel":"bbc1","start":"${fmt.print(now)}","end":"${fmt.print(now.plusHours(2))}","category":["documentary"],"accessibility":["flags1"],"series":{"serieTitle":"serie1","episodeTitle":"ep1","description":null,"seasonNumber":null,"episodeNumber":null,"totalNumber":null},"program":{"title":"program1","description":null},"id":"$idString"}"""
    }
    "transform json to TVProgram object" in {
      Json.parse(s"""{"channel":"bbc1","start":"${fmt.print(now)}","end":"${fmt.print(now.plusHours(2))}","category":["documentary"],"accessibility":["flags1"],"series":{"serieTitle":"serie1","episodeTitle":"ep1","description":null,"seasonNumber":null,"episodeNumber":null,"totalNumber":null},"program":{"title":"program1","description":null},"id":"$idString"}""")
