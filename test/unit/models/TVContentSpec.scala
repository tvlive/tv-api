...
class TVContentSpec extends PlaySpec with MustMatchers {
  ...
  "onTimeNow" should {
    "be true when tvContent start time is before now and after time is after now" in {
      val content = TVContent("channel1", List("provider1"), now.minusHours(1), now.plusHours(1), List(), None, None, None, None)
      content.onTimeNow mustBe true
    }

    "be false when tvContent start time and after time before now" in {
      val content = TVContent("channel1", List("provider1"), now.minusHours(2), now.minusHours(1), List(), None, None, None, None)
      content.onTimeNow mustBe false
    }

    "be false when tvContent start time and after time after now" in {
      val content = TVContent("channel1", List("provider1"), now.plusHours(1), now.plusHours(2), List(), None, None, None, None)
      content.onTimeNow mustBe false
    }
  }
  ...
  "perCentTimeElapsed" should {
    ...
    "be 20% when tvContent started 1 hour ago and finishes in 4 hour" in {
      val content = TVContent("channel1", List("provider1"), now.minusHours(1), now.plusHours(4), List(), None, None, None, None)
    ...
    "be None when tvContent starts in 2 hours and finishes in 3 hour" in {
      val content = TVContent("channel1", List("provider1"), now.plusHours(2), now.plusHours(3), List(), None, None, None, None)
      content.perCentTimeElapsed mustBe None
    }
  }
  ...
  "channelImageURL" should {
    ...
    "be channel_exampe.png when the channel is channel example" in {
      val content = TVContent("channel example", List("provider1"), now.plusHours(2), now.plusHours(3), List(), None, None, None, None)
