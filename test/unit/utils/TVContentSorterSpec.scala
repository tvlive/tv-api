...
class TVContentSorterSpec extends PlaySpec with MustMatchers with TVContentSorter {
  ...
  "sortedByRatingTimeChannel" should {
    ...
    "return true when the first has rating and second has no rating" in {
      ...
      sortedByRatingTimeChannel(f, s) mustBe true
    }
  ...
  "sortedByTimeRatingChannel" should {

    "return true when the first started before" in {
      val f = TVContent(channel = "channel1", provider = List("provider1"), start = current.plusHours(1),
        end = current.plusHours(2), rating = Some(7.5), None, None, None, None)
      ...
      sortedByTimeRatingChannel(f, s) mustBe true
    }


    "return true when the first same time than second but more rating" in {
      val f = TVContent(channel = "channel1", provider = List("provider1"), start = current.plusHours(1),
        end = current.plusHours(2), rating = Some(7.5), None, None, None, None)
    ...
    "return true when the first same time than second but second has no rating" in {
      val f = TVContent(channel = "channel1", provider = List("provider1"), start = current.plusHours(1),
        end = current.plusHours(2), rating = Some(7.5), None, None, None, None)
    ...
    "return true when the first same time and rating than second but the channel name of the first is less or eq than the second" in {
        val f = TVContent(channel = "channel1", provider = List("provider1"), start = current.plusHours(1),
          end = current.plusHours(2), rating = Some(7.5), None, None, None, None)
